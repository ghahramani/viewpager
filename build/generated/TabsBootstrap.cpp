...
#define TAG "com.navid.tabs"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Tabs_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Tabs.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::TabsBindings::lookupGeneratedInit(
...
static void Tabs_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Tabs_getBinding)->GetFunction());
}

static void Tabs_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TabsBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TabsBinding = {
	"com.navid.tabs",
	Tabs_init,
	Tabs_dispose
};
...
Java_com_navid_tabs_TabsBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.navid.tabs", &TabsBinding);
	titanium::KrollBindings::addExternalLookup(&(::TabsBindings::lookupGeneratedInit));
}
